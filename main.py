...
class DataAnalyzer:
    ...
    def __init__(self, config: dict):
        ...
        self.max_packets = self._get_int(config, 'max_packets', 10, 1)
        self.max_packet_messages = self._get_int(config, 'max_packet_messages', 50, 1)
    ...
    def _log(self, level: int, message: str, indent: int = 0):
        """Логирование с учётом уровня и отступов."""
    ...
    def step1_analyze_chats(self):
        ...
        """
        self._log(1, "Запуск шага 1: анализ чатов", indent=1)
        ...
        for chat in chats:
            ...
            self._log(2, f"\n  Чат '{chat_name}' (ID: {chat_id}) исходно разбит на {len(all_packets)} пакетов.", indent=2)
            ...
            if self.max_packets is not None and self.max_packets > 0 and len(all_packets) > self.max_packets:
                ...
                self._log(2, f"    Ограничение max_packets={self.max_packets}: взято последних {len(packets_to_process)} пакетов.", indent=3)
            else:
            ...
            for idx, packet_msgs in enumerate(packets_to_process, start=1):
                ...
                if self.max_packet_messages is not None and self.max_packet_messages > 0 and len(packet_msgs) > self.max_packet_messages:
                    ...
                    self._log(2, f"      Пакет {idx}: ограничение max_packet_messages={self.max_packet_messages}, взято последних {len(packet_msgs)} сообщений.", indent=4)
                ...
                self._log(2, f"\n      --- Обработка пакета {idx} (чат: {chat_name}) ---", indent=4)
    ...
    def step2_aggregate_results(self):
        ...
        """
        self._log(1, "Запуск шага 2: агрегация результатов", indent=1)
        step1_json_path = "agent_data/analysis_step_1.json"
    ...
    def step3_backward_mapping(self):
        """
        ...
        """
        self._log(1, "Запуск шага 3: обратный проход", indent=1)
        step1_json_path = "agent_data/analysis_step_1.json"
        step2_json_path = "agent_data/analysis_step_2.json"
